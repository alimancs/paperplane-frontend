...
const PostCardMobile:React.FC<components> = ( { timestamp, title, comments, likes, savedByClient, summary, image, slug } ) => {
    const options = [
    ...
    return (
        <Link to={`/@username/${slug}`} className={`flex md:hidden border-b-[0.5px] border-gray-200 flex-col w-full px-5 text-gray-900hover:cursor-pointer justify-center items-start h-[270px]`}>
            ...
            <div className="w-full flex flex-row h-fit  justify-between">
                ...
                <img className="w-[25%] h-[60px]" src={image}/>
            </div>
