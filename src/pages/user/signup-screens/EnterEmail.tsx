...
const EnterEmail:React.FC<props> = ( { email, setEmail, nextScreen, toHome }) => {
    ...
    const displayError = (errMsg:string) => {
        const inputBox = document.getElementById('emailInput');
        if (inputBox) {
    ...
    const handleSendOTP = async () => {
        ...
        const data = { email };
        try {
            const response = await axios.post('http://localhost:3000/api/auth/otp', data);
        ...
        <div className="flex-1 flex gap-3 flex-col justify-center items-center text-[#17124f]">
            { !sendingEmail &&  (
                <div className="flex flex-col mt-[20px] md:w-[400px] w-[80%] gap-2">
                    ...
                    <button onClick={sendOtpAndVerify} className="px-[20px] mx-auto w-[100%] text-center mt-[15px] text-[14px] flex flex-row justify-center items-center gap-3 text-[#ecf0f1] h-[35px] cursor-pointer hover:scale-105 transition-all duration-300 ease-in-out bg-[#17124f]">
                        Continue
                    </button>
